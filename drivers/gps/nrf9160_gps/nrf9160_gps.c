...
static void data_handler_thread_fn(int dev_ptr)
{
	...
	while (true) {
		...
		case NRF_MODEM_GNSS_EVT_PVT:
			...
			// TK: This is for periodic mode only?
			// BUG: SEARCH_STARTED sent twice when periodic started after a fix (already
			// fixed?)
			if (atomic_get(&drv_data->timeout_occurred) ||
			    (drv_data->config.interval != 1 && fix_valid)) {
		...
		case NRF_MODEM_GNSS_EVT_NMEA:
			...
			memcpy(evt.nmea.buf, nmea_data->nmea_str, nmea_len);
...
static void thread_init(const struct device *dev)
{
	struct gps_drv_data *drv_data = dev->data;
	...
	drv_data->thread_id =
		k_thread_create(&drv_data->thread, drv_data->thread_stack,
				...
				(k_thread_entry_t)data_handler_thread_fn, (void *)dev, NULL, NULL,
		     ...
		     struct gnss_config *cfg_dst)
{
	...
	/* This is the only start mode currently supported */
...
static void blocked_work_fn(struct k_work *work)
{
	...
	retval = nrf_modem_gnss_prio_mode_enable();
	if (retval != 0) {
		LOG_ERR("Failed to enable GPS priority, error: %d", retval);
		return;
	}
...
static int init(const struct device *dev, gps_event_handler_t handler)
{
	...
	if (err) {
		LOG_ERR("Failed to initialize GNSS API, error: %d", err);
		return err;
	}
...
static int start(const struct device *dev, struct gps_config *cfg)
{
	...
	if (atomic_get(&drv_data->is_running)) {
		LOG_ERR("GPS is already active");
		return -EPERM;
	}

	err = parse_cfg(cfg, &drv_data->config);
	if (err) {
	...
	retval = nrf_modem_gnss_fix_retry_set(drv_data->config.retry);
	if (retval != 0) {
		LOG_ERR("Failed to set fix retry value, error: %d", retval);
		return -EIO;
	}

	retval = nrf_modem_gnss_fix_interval_set(drv_data->config.interval);
	if (retval != 0) {
		LOG_ERR("Failed to set fix interval value, error: %d", retval);
		return -EIO;
	}

	retval = nrf_modem_gnss_nmea_mask_set(drv_data->config.nmea_mask);
	if (retval != 0) {
		LOG_ERR("Failed to set NMEA mask, error: %d", retval);
		return -EIO;
	}
	...
	retval = nrf_modem_gnss_power_mode_set(drv_data->config.power_mode);
	if (retval != 0) {
		LOG_ERR("Failed to set GPS power mode, error: %d", retval);
		return -EIO;
	}


	retval = nrf_modem_gnss_use_case_set(drv_data->config.use_case);
	if (retval != 0) {
		LOG_ERR("Failed to set GPS use case, error: %d", retval);
		return -EIO;
	}

	if (drv_data->config.delete_mask != 0) {
		retval = nrf_modem_gnss_nv_data_delete(drv_data->config.delete_mask);
		if (retval != 0) {
			LOG_ERR("Failed to delete NV data, error: %d", retval);
			return -EIO;
		}
	}
	...
	if (retval != 0) {
		LOG_ERR("Failed to start GPS, error: %d", retval);
		return -EIO;
	}
...
static int stop(const struct device *dev)
{
	...
	// TK: Check and remove comment if necessary
